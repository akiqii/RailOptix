...
def export_csv_bundle(scenario_letter: str):
    ...
    if scen == "A":
        reqs = OPTIMIZER.build_scenario_a(BASELINE_REQUESTS).requests
    elif scen == "B":
        reqs = OPTIMIZER.build_scenario_b(BASELINE_REQUESTS).requests
    else:
        reqs = OPTIMIZER.build_scenario_c(BASELINE_REQUESTS).requests

    zip_data = EXPORTER.create_zip_archive(scen, reqs)
    return Response(
        content=zip_data,
        media_type="application/zip",
        headers={"Content-Disposition": f"attachment; filename=RailOptix_Scenario_{scen}_Deliverables.zip"}
    )
...
def download_scenario_deliverable(scenario_letter: str, filename: str):
    ...
    return Response(
        content=content,
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename={scen}_{filename}"}
    )
